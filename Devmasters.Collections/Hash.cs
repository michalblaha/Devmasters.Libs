...
/* Unmerged change from project 'Devmasters.Collections (net472)'
Before:
    ...
    internal class Hash<T> : IEnumerable<T>, ISerializable, IDeserializationCallback
    {
        ...
        struct Slot
        {
            ...
            public int HashValue
            {
                ...
                set
                {
                    //System.Diagnostics.Debug.Assert((value & 0x80000000) == 0);  // make sure sign bit isn't set.
        ...
        private void ResizeTable(int newSize)
        {
            ...
            //System.Diagnostics.Debug.Assert((newSize & (newSize - 1)) == 0);            // Check newSize is a power of two.
            totalSlots = newSize;
        ...
        {
            System.Diagnostics.Debug.Assert(count <= usedSlots);
            System.Diagnostics.Debug.Assert(count <= totalSlots);
            System.Diagnostics.Debug.Assert(usedSlots <= totalSlots);
            System.Diagnostics.Debug.Assert(usedSlots <= thresholdGrow);
            System.Diagnostics.Debug.Assert((int)(totalSlots * loadFactor) == thresholdGrow);
            if (thresholdShrink > 1)
                System.Diagnostics.Debug.Assert(thresholdGrow / 3 == thresholdShrink);
            else
                System.Diagnostics.Debug.Assert(thresholdGrow / 3 <= MINSIZE);
            if (totalSlots > 0)
            {
                System.Diagnostics.Debug.Assert((totalSlots & (totalSlots - 1)) == 0);  // totalSlots is a power of two.
                System.Diagnostics.Debug.Assert(totalSlots - 1 == hashMask);
                System.Diagnostics.Debug.Assert(GetSecondaryShift(totalSlots) == secondaryShift);
                System.Diagnostics.Debug.Assert(totalSlots == table.Length);
            }
            ...
            if (table != null)
            {
                for (int i = 0; i < totalSlots; ++i)
                {
                    ...
                    if (slot.Empty)
                    {
                        ...
                        if (slot.Collision)
                            ++expectedUsed;
                        System.Diagnostics.Debug.Assert(object.Equals(default(T), slot.item));
                    }
                    else
                    {
                        // not empty.
                        ++expectedCount;
                        ++expectedUsed;
                        System.Diagnostics.Debug.Assert(slot.HashValue != 0);
                        System.Diagnostics.Debug.Assert(GetHashValues(slot.item, out initialBucket, out skip) == slot.HashValue);
                        if (initialBucket != i)
                            System.Diagnostics.Debug.Assert(table[initialBucket].Collision);
                    }
                }
            }

            System.Diagnostics.Debug.Assert(expectedCount == count);
            System.Diagnostics.Debug.Assert(expectedUsed == usedSlots);
        }
